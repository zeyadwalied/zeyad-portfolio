class HeroAutoMorph {
    ...
    animate() {
        ...
        for (let i = 0; i < this.particles.length; i++) {
            ...
            const morphNoiseX = Math.sin(Date.now() * 0.0015 + p.angleOffset) * 0.014;
            const morphNoiseY = Math.cos(Date.now() * 0.0012 + p.angleOffset) * 0.014;
            const floatNoise = Math.sin(Date.now() * 0.001 + p.angleOffset) * 0.09;
